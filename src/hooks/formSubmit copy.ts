  ...
  */
  const submit = async (formData?: T) => {
    ...
    try {
      ...
      //si hay parametros que agregar en la url se los agregamos
      ...
      if(responseData && responseData.user && responseData.token){//en caso de ser un login o un register exitoso
        setToken(responseData.token)
        setUsuario(responseData.user)
        localStorage.setItem('vc-token',responseData.token)
      }
      if(responseData.message === "Usuario actualizado con exito"){//en caso de ser un edit de usuario exitoso
